...
static ErrorCode parse_JSON(Dipoles *dipoles)
{
    ...
    config.mod_k = (2.0 * M_PI) / (double) json_object_get_int(wavelength);
    ...
    config.scatterer_size[Y] = json_object_get_int(json_object_array_get_idx(scatterer_size, 1));
    config.scatterer_size[Z] = json_object_get_int(json_object_array_get_idx(scatterer_size, 2));
...
static ErrorCode unpack_input_fields(InputField **input_fields)
{
    ...
    *input_fields = (InputField *) malloc(config.num_inputs * sizeof(InputField));
    ...
    for (int input_num = 0; input_num < config.num_inputs; input_num++) {
        ...
        (*input_fields)[input_num].k_space = (gsl_complex_packed_array) malloc(config.scatterer_size[X] * config.scatterer_size[Y] * sizeof(gsl_complex));
        ...
        /* Takes strtok up to the next space, leaving the Y dimension of the focus point in temp_string. */
        temp_string = strtok(NULL, " ");
        STRTOK_CHECK(temp_string, filepath, in_file);
        (*input_fields)[input_num].focus_point[Y] = atoi(temp_string);

        /* Z component of focus point must be on the detector. */
        (*input_fields)[input_num].focus_point[Z] = config.detector_z;

        /* Read input field data into array, assuming 2D field is flattened into a 1D array. */
        for (int i = 0; i < config.scatterer_size[X]*config.scatterer_size[Y]; i++) {
            line = fgets(line_buffer, sizeof(line_buffer), in_file);
            if (feof(in_file) || (strcmp(line, "") == 0)) {
        ...
        // for (int i = 0; i < 512; i++) {
        //     for (int j = 0; j < 512; j++) {
        //         printf("%.1f ", (*input_fields)[input_num].k_space[FIELD_ACCESS(i, j)]);
        //     }
        //     printf("\n");
        // }

        // /* Apply FFT to rows. */
        // for (int i = 0; i < config.scatterer_size[Y]; i++) {
        //     gsl_fft_complex_radix2_forward(&(*input_fields)[input_num].k_space[2 * i * config.scatterer_size[X]], 1, config.scatterer_size[X]);
        // }

        // /* Apply FFT to columns. */
        // for (int j = 0; j < config.scatterer_size[X]; j++) {
        //     gsl_fft_complex_radix2_forward(&(*input_fields)[input_num].k_space[2 * j], config.scatterer_size[X], config.scatterer_size[Y]);
        // }
    }
...
static gsl_complex_packed_array evaluate_total_field_at_plane(Dipoles dipoles, InputField input_field, unsigned int z)
{
    ...
    for (int x = 0; x < config.scatterer_size[X]; x++) {
        ...
        for (int y = 0; y < config.scatterer_size[Y]; y++) {
            ...
            GSL_SET_REAL(&E, PACKED_REAL(field, FIELD_ACCESS(x, y)));
            GSL_SET_IMAG(&E, PACKED_IMAG(field, FIELD_ACCESS(x, y)));
...
static gsl_complex_packed_array propagate_k_field_to_plane(InputField input_field, unsigned int z)
{
    ...
    out_field = (gsl_complex_packed_array) malloc(config.scatterer_size[X] * config.scatterer_size[Y] * sizeof(gsl_complex));
    ...
    for (int x = 0; x < config.scatterer_size[X]; x++) {
        ...
        for (int y = 0; y < config.scatterer_size[Y]; y++) {
            ...
            PACKED_SET_REAL(out_field, FIELD_ACCESS(x, y), GSL_REAL(k_field_value));
            PACKED_SET_IMAG(out_field, FIELD_ACCESS(x, y), GSL_IMAG(k_field_value));
        }
    }

    /* Now, find the inverse FFT of the k-space field to yield the real field at the correct plane. */

    /* Apply inverse FFT to rows. */
    for (int i = 0; i < config.scatterer_size[Y]; i++) {
        gsl_fft_complex_radix2_inverse(&out_field[2 * i * config.scatterer_size[X]], 1, config.scatterer_size[X]);
    }

    /* Apply inverse FFT to columns. */
    for (int j = 0; j < config.scatterer_size[X]; j++) {
        gsl_fft_complex_radix2_inverse(&out_field[2 * j], config.scatterer_size[X], config.scatterer_size[Y]);
    }
...
static gsl_complex propagate_k_field_to_k_point(InputField input_field, int point[DIMS])
{
    ...
    GET_KX(&k_y, config.scatterer_size[Y], point[Y]);

    sqrt_arg = (config.mod_k * config.mod_k) - (k_x * k_x) - (k_y * k_y);
    ...
    original = gsl_complex_rect(PACKED_REAL(input_field.k_space, FIELD_ACCESS(point[X], point[Y])), PACKED_IMAG(input_field.k_space, FIELD_ACCESS(point[X], point[Y])));
...
static gsl_complex evaluate_input_field_at_point(InputField input_field, int point[DIMS])
{
    ...
    GSL_SET_REAL(&E, PACKED_REAL(field_at_plane, FIELD_ACCESS(point[X], point[Y])));
    GSL_SET_IMAG(&E, PACKED_IMAG(field_at_plane, FIELD_ACCESS(point[X], point[Y])));
...
static double evaluate_virtual_dipole(Dipole *virtual_dipole, Dipoles dipoles, InputField input_field, InputField *all_input_fields)
{
    ...
    for (int i = 0; i < dipoles.num_dipoles; i++) {
        ...
        if (i == virtual_dipole->_id) { /* Virtual dipole has same ID as its primary dipole. */
            vector_between[X] = virtual_dipole->position[X] - input_field.focus_point[X];
            vector_between[Y] = virtual_dipole->position[Y] - input_field.focus_point[Y];
            ...
            vector_between[Y] = dipoles._dipoles[i].position[Y] - input_field.focus_point[Y];
        ...
        */
        ...
        for (int y = -1; y <= 1; y++) {
            ...
            for (int x = -1; x <= 1; x++) {
                ...
                /* First, retrieve E at (x, y, z) from complex packed array. */
                GSL_SET_REAL(&E, PACKED_REAL(z_plane_field, FIELD_ACCESS(virtual_dipole.position[X], virtual_dipole.position[Y])));
                GSL_SET_IMAG(&E, PACKED_IMAG(z_plane_field, FIELD_ACCESS(virtual_dipole.position[X], virtual_dipole.position[Y])));
...
static ErrorCode evaluate_progress(Dipoles *dipoles, InputField *input_fields, int epoch_num)
{
    ...
    for (int input_num = 0; input_num < config.num_inputs; input_num++) {
        ...
        fitness += gsl_hypot(
            PACKED_REAL(field_on_detector, FIELD_ACCESS(input_fields[input_num].focus_point[X], input_fields[input_num].focus_point[Y])),
            PACKED_IMAG(field_on_detector, FIELD_ACCESS(input_fields[input_num].focus_point[X], input_fields[input_num].focus_point[Y]))
        );
    }
