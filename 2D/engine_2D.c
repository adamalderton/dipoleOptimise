...
static double evaluate_virtual_dipole(Dipole *virtual_dipole, Dipoles dipoles, SourceDipole source_dipole, SourceDipole *all_source_dipoles);
...
static ErrorCode process_results(Dipoles dipoles, SourceDipole *source_dipoles)
{
    ...
    for (int input_num = 0; input_num < config.num_inputs; input_num++) {
        ...
        for (int z = 0; z < config.scatterer_size[Z] + config.extra_z_evaluation; z += config.resolution_factor) {
            ...
            gsl_complex_packed_array lattice_field = evaluate_total_field_at_plane(dipoles, source_dipoles[input_num], z);
...
static gsl_complex_packed_array evaluate_total_field_at_plane(Dipoles dipoles, SourceDipole source_dipole, unsigned int z)
{
    ...
    field = (gsl_complex_packed_array) malloc(config.scatterer_size[X] * sizeof(gsl_complex));
    ...
    for (int x = 0; x < config.scatterer_size[X]; x++) {
        ...
        /* Loop over other dipoles, adding contribution from each. */
        ...
        PACKED_SET_REAL(field, x, GSL_REAL(E));
        PACKED_SET_IMAG(field, x, GSL_IMAG(E));
    }
...
static double evaluate_virtual_dipole(Dipole *virtual_dipole, Dipoles dipoles, SourceDipole source_dipole, SourceDipole *all_source_dipoles)
{
    ...
    /* Find intensity on focus point due to source dipole only. */
    VD_total_field = evaluate_input_field_at_point(source_dipole, source_dipole.focus_point);
    ...
    for (int i = 0; i < dipoles.num_dipoles; i++) {
        ...
        if (i == virtual_dipole->_id) { /* Virtual dipole as same ID as its primary dipole. */
            ...
            vector_between[X] = dipoles._dipoles[i].position[X] - source_dipole.focus_point[X];
            vector_between[Z] = dipoles._dipoles[i].position[Z] - source_dipole.focus_point[Z];
        }
    ...
    */
    for (int z = -1; z <= 1; z++) {
        ...
        for (int x = -1; x <= 1; x++) {
             ...
             */
            primary_dipole->vdfs[virtual_dipole_num] += (float) evaluate_virtual_dipole(&virtual_dipole, dipoles, source_dipole, all_source_dipoles);
...
static ErrorCode evaluate_progress(Dipoles *dipoles, SourceDipole *all_source_dipoles, int epoch_num)
{
    ...
    for (int input_num = 0; input_num < config.num_inputs; input_num++) {
        ...
        fitness += gsl_hypot(
            PACKED_REAL(field_on_detector, all_source_dipoles[input_num].focus_point[X]),
            PACKED_IMAG(field_on_detector, all_source_dipoles[input_num].focus_point[X])
        );
    }
